  ...
  return (
    <>
      <Shell step={step} onStep={setStep} onReset={reset} sessionId={session.sessionId}>
        ...
        {step === 4 && <PredictionScreen diseaseName={disease.title} previous={() => setStep(3)} next={() => setStep(5)} />}
...
function PredictionScreen({ diseaseName, previous, next }: { diseaseName: string; previous: () => void; next: () => void }) {
  return (
    <div className="screen screen--prediction">
      ...
      <div className="prediction-lower">
        ...
        <aside className="panel research-note"><h2>연구 메모</h2><p><Brain /> 선택 질환 <strong>{diseaseName}</strong></p><p><Microscope /> 샘플 종류 <strong>3D 오가노이드</strong></p><p><Target /> 단백질 표적 <strong>EGFR</strong></p><NavButtons previous={previous} next={next} nextLabel="후보물질 비교" /></aside>
      </div>
